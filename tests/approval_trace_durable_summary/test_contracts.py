...
class ContractTests(unittest.TestCase):
 ...
 def test_rejects_invalid_schema_profile_and_safe_text(self):
  ...
  with self.assertRaises(ValueError): MetadataArtifactReference(SCHEMA,D,"run-0001",D,D,D,D,"profile-001",0,D)
  with self.assertRaises(ValueError): MetadataArtifactReference(SCHEMA,D,"run-0001",D,D,D,D,"profile-001",True,D)
  with self.assertRaises(ValueError): MetadataArtifactReference(SCHEMA,D,"run-0001"*40,D,D,D,D,"profile-001",1,D)
 def test_reference_trace_and_summary_are_frozen_and_reject_payloads(self):
  reference=MetadataArtifactReference(SCHEMA,D,"run-0001",D,D,D,D,"profile-001",1,D)
  ...
  with self.assertRaises(ValueError): MetadataArtifactReference(SCHEMA,D,"run/path",D,D,D,D,"profile-001",1,D)
